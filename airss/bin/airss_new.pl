...
for ($i = 1; $i <= $opt_max; $i++) {
  ...
  if ( ! $opt_mode || $j == 0) {
	...
	if ( $opt_repose ) {
	  $spe=`(repose_relax 'repose_new -p $opt_pressure -dmin $opt_devmin -dmax $opt_devmax' $opt_mpinp $opt_workdir/$unique) | awk '/Enthalpy:/ {print \$2}'`;
  ...
  if ( $opt_pp0 ) {
    ...
    @results=`(repose_relax 'repose_new -dmin $opt_devmin -dmax $opt_devmax' $opt_mpinp $opt_workdir/$unique)`;
 } elsif ( $opt_ramble )  {
