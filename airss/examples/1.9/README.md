...
host:1.9 cjp10$ cat AB.cell
#VARVOL=15
#SPECIES=A,B
#NATOM=2-8
#MINSEP=1.5
host:1.9 cjp10$ airss.pl -pp3 -max 100 -seed AB

If the qhull package has been installed, we can compute the binary convex hull (or Maxwell construction).

host:1.9 cjp10$ ca -m | sort -n -k 6 -k 5
...
Using xmgrace the hull.agr file generate can be examined. 

host:1.9 cjp10$ xmgrace hull.agr

The end members for the convex hull calculation can be specified. Stable compounds are denoted by a "+" symbol.

host:1.9 cjp10$ ca -m -1 BA -2 B | sort -n -k 6 -k 5
