...
99  continue
  ...
  subroutine read_castep()
    ...
    do n=1,num_lines
       ...
       if(index(buff(n),'Charge(e)   Spin(hbar/2)').gt.0) then
          do ni=1,num_ions
  ...
  subroutine read_res()
    ...
    if(num_words(1).ne.12) then

       structure_name='unknown'
       pressure=0.0_dp
       lattice_volume=0.0_dp
       enthalpy=0.0_dp
       total_spin=0.0_dp
       total_modspin=0.0_dp
       spacegroup='n/a'
       ncopies=1
  ...
  subroutine write_res()
    ...
    character(len=100) :: cpress,cvol,centh
    ...
    write (cpress,'(f40.12)') pressure 
    write (cvol,'(f40.12)') lattice_volume*factor
    write (centh,'(f40.12)') enthalpy*factor

    write (stdout,'(a,1x,a,1x,a,1x,a,1x,a,2f6.2,1x,a,a,i5)') 'TITL',trim(structure_name),trim(adjustl(cpress)),&
         trim(adjustl(cvol)),trim(adjustl(centh)),total_spin*factor,total_modspin*factor,trim(adjustl(ctemp2)),&
...
!!$    write (stderr,*)
  ...
  subroutine read_xyze()
    ...
    integer :: ni,ind,indx0,indx1,indx2,nfu
    ...
    indx0=index(buff(2),'attice=')

    ind=indx0
    do
       ind=ind+1
       if(buff(2)(ind:ind).eq.'"') then
          indx1=ind
          exit
       end if
    end do
    ...
    indx0=index(buff(2),'space_group=')
    ...
    if(indx0.gt.0) then
       ...
       read(buff(2)(indx1+1:indx2-1),*) spacegroup

       spacegroup=deunder(trim(spacegroup))

    end if

    ! ** Read the energy (enthalpy???)

    indx0=index(buff(2),' minPE=')

    if(indx0.gt.0) read(buff(2)(indx0+7:),*) enthalpy

    ! ** Read the number of formula units -- note, this is not the actual number of formula units.

    indx0=index(buff(2),' No_fu=')

    if(indx0.gt.0) read(buff(2)(indx0+7:),*) nfu

    enthalpy=enthalpy*nfu

    ! ** Read the label

    indx0=index(buff(2),' label=')

    if(indx0.gt.0) read(buff(2)(indx0+7:),*) structure_name
  ...
  subroutine write_qe()
    ...
    integer :: n,ns,nispc(num_species)

    character(len=40) :: cint(num_ions),cspin,ctmp

    nispc=0
    do ns=1,num_species
       do n=1,num_ions
          if(ion_index(n).eq.ns) then
             if(ion_spin(n).ne.0.0_dp) nispc(ns)=nispc(ns)+1
          end if
       end do
    end do
    ...
    write(ctmp,*) num_ions
    write (stdout,'(a)') '  nat='//trim(adjustl(ctmp))//','
    write (ctmp,*) sum(nispc)
    write (stdout,'(a)') '  ntyp='//trim(adjustl(ctmp))//','
    ! Decide whether we have nspin = 1 (non-spin-polarized calculation) or nspin = 2 (spin-polarized calculation)
    if(any(ion_spin.ne.0.0_dp)) then
       write (stdout,'(a)') '  nspin=2'
    else
       write (stdout,'(a)') '  nspin=1' 
    end if

    do n=1,num_ions
       write(cint(n),'(i5)') n
       write(cspin,'(f20.14)') ion_spin(n)
       if(ion_spin(n).ne.0.0_dp) write (stdout,*) ' starting_magnetization('//trim(adjustl(cint(n)))//') = '//trim(adjustl(cspin))
    end do
    write (stdout,*)
    ...
    do n=1,num_ions
       if(ion_spin(n).ne.0.0_dp) then
          write (stdout,'(a3,4f20.14)') adjustl(trim(species_names(ion_index(n))))//trim(adjustl(cint(n))),ion_fractional(:,n)
       else
          write (stdout,'(a3,3f20.14)') adjustl(trim(species_names(ion_index(n)))),ion_fractional(:,n)
       end if
    end do
