...
program cryan
  ...
  integer                                         :: num_species
  integer                                         :: num_points
  integer                                         :: formula_unit=0
  integer                                         :: element_index(118)
  integer                                         :: ns,i,j,k,na,nb,nc,nt,n,nn,seed_indx,stat,l,nin,ninc,ne,natmpt,nas,nbs,nh
  integer                                         :: n1,n2,nrep,dof,indx1,indx2,num_penth,num_pestruc,ntrain,indx,nii,naa,nbb
  integer                                         :: nref(1),num_struc,specnum=0
  ...
  integer,            allocatable, dimension(:,:) :: cvec,cvec2
  ...
  real(kind=dp)                                   :: comp_threshold,elim_threshold=0.0_dp,enth,temp,maxq
  real(kind=dp)                                   :: lscale,dimensionality=-1.0_dp
  ...
  real(kind=dp),    allocatable, dimension(:,:)   :: maxdiff,LL,MM,SS,B,clustdist,unit_save,comp_frac
  real(kind=dp),    allocatable, dimension(:,:)   :: hull_normal,etrain,enref,comp_vec
  real(kind=dp),    allocatable, dimension(:,:,:) :: peinterp


  character(len=20)                               :: task(100)
  character(len=10)                               :: species_names(118)
  character(len=40)                               :: formula='',formula_convert='',formula_ref=''
  character(len=40), dimension(200,4)             :: comp_endmember=''
  ...
  character(len=5)                                :: ctemp5 
  character(len=240)                              :: ctemp,ctemp2,ctemp3,unitfile,fmt,label
  character(len=4),   dimension(200)              :: element_list=''
  ...
  character(len=10)                               :: grace_output_type
  character(len=10)                               :: grace_output_suffix
  character(len=20)                               :: hull_prefix
  character(len=40)                               :: qconv_in,qconv_out
  ...
  type :: crystal
     ...
     integer,           allocatable, dimension(:,:)   :: dist_Z
     integer,           allocatable, dimension(:,:)   :: adjacancy
  ...
  do nt=1,num_tasks
     ...
     case('formula_unit') ; call formula_unit_pick()
     ...
     case('vector') ; call output_vector()
...
contains
  ...
  subroutine read_buff()
    ...
    character(len=240), allocatable, dimension(:)   :: tmpbuff
    ...
    resbuff(1)='##start##'
    do
       read(5,'(a)',end=99,err=100) ctemp
       if(ctemp(1:3).eq.'REM') cycle
       
       if(num_lines.ge.size(resbuff)) then
          call move_alloc(resbuff, tmpbuff)
          allocate(resbuff(num_lines*2))
          resbuff(1:num_lines) = tmpbuff
       end if
       num_lines=num_lines+1
       
       resbuff(num_lines) = ctemp
    end do
100 stop
99  continue
  ...
  subroutine read_res()
    ...
    do while (nend.lt.num_lines)
       ...
       if(nstart.ge.num_lines) goto 444
       ...
       select case(ntokens) 
       case(9)
          read(tokens(2),*,err=444) structure(ns)%structure_label
          read(tokens(3),*,err=444) structure(ns)%pressure
          read(tokens(4),*,err=444) structure(ns)%volume
          read(tokens(5),*,err=444) structure(ns)%enthalpy
          read(tokens(6),*,err=444) structure(ns)%spin
          read(tokens(7),*,err=444) structure(ns)%modspin
          read(tokens(8),*,err=444) structure(ns)%del
          read(tokens(9),*,err=444) structure(ns)%num_ions
       case(8)
          read(tokens(2),*,err=444) structure(ns)%structure_label
          read(tokens(3),*,err=444) structure(ns)%pressure
          read(tokens(4),*,err=444) structure(ns)%volume
          ...
          read(tokens(6),*,err=444) structure(ns)%spin
          read(tokens(7),*,err=444) structure(ns)%modspin
          read(tokens(8),*,err=444) structure(ns)%num_ions
       case(7)
          read(tokens(2),*,err=444) structure(ns)%structure_label
          read(tokens(3),*,err=444) structure(ns)%pressure
          read(tokens(4),*,err=444) structure(ns)%volume
       ...
       case(4)
          read(tokens(2),*,err=444) structure(ns)%structure_label
          read(tokens(3),*,err=444) structure(ns)%volume
          read(tokens(4),*,err=444) structure(ns)%enthalpy
       case default
          continue
       end select
       ...
       read(resbuff(nstart+nrem+1),*,err=444,end=444) ctemp,ctemp,structure(ns)%lattice_abc(1:6)
       ...
       ! ** Check structure exists in the chosen chemical space

       if(any(len_trim(element_list).gt.0)) then
       
          do ni=1,structure(ns)%num_species          
             if(.not.any(structure(ns)%species_names(ni).eq.element_list)) then
                structure(ns)%num_copies=0
                exit
             end if
          end do
       ...
       ! ** Filer for number of species

       if((structure(ns)%num_species.ne.specnum).and.(specnum.gt.0)) structure(ns)%num_copies=0
...
444    continue
       ...
       write (stderr,*) 'read_res: parsing structure failed: ',trim(resbuff(nstart))
  ...
  subroutine formula_unit_pick()
    
    do ns=1,num_structures
       if(structure(ns)%num_copies==0) cycle
       if (structure(ns)%num_form.ne.formula_unit) structure(ns)%num_copies=0
    end do
    
  end subroutine formula_unit_pick
             ...
             do nc=1,structure(ns)%num_units
                ...
                do i=1,structure(ns)%num_ions
                   if(structure(ns)%in_unit(i).eq.nc) then
                      ...
                      ! ** Tag broken connectivity

                      do j=1,structure(ns)%num_ions
                         if((structure(ns)%adjacancy(i,j).gt.0).and.(structure(ns)%in_unit(i).ne.structure(ns)%in_unit(j))) then
                            structure(ns)%unit_names(nin,nc)=trim(structure(ns)%unit_names(nin,nc))//"'"
                            structure(ns)%ion_names(i)=trim(structure(ns)%ion_names(i))//"'"
                         end if
                      end do
                ...
                if(structure(ns)%num_unit(nc).gt.0) then
                   ...
                   if(notsymm) then
                      ...
                      if(structure(ns)%num_unit(nc).gt.2) then
                         ...
                         structure(ns)%unit_pointgp(nc)="C1"
                         read(unit_unit,*,end=343,err=343) structure(ns)%unit_pointgp(nc)
343                      if(structure(ns)%unit_pointgp(nc).eq."C1") then
                            structure(ns)%unit_num_asym(nc)=structure(ns)%num_unit(nc)
                         else
          ...
          do nc=1,structure(ns)%num_units
             ...
             do i=1,structure(ns)%num_unit(nc)
                write(ctemp4,'(a)') trim(structure(ns)%unit_names(i,nc))
                write (stdout,'(a5,3f10.5,a,a,a)') &
                     ctemp4,structure(ns)%units_cart(:,i,nc),' # ',&
          ...
          write (stdout,'(a)') '#CELLADAPT'

          if(max(bondlength,bondscale).gt.0.0_dp) deallocate(LL,MM,SS,B,E,S,So,work,iwork,iwork2,taken,in,dcom,dia,eab)
    ...
    do ns=1,num_structures
       if(structure(ns)%num_copies==0) cycle
       do n=1,structure(ns)%num_ions
          if(all(structure(ns)%ion_names(n).ne.species_names(1:num_species))) then
             num_species=num_species+1
             species_names(num_species)=structure(ns)%ion_names(n)
          end if
       end do
    end do
    ...
    do ns=1,num_structures
       ...
       do na=1,num_species
          do nb=na,num_species
             ...
             if(n.le.structure(ns)%num_dist) then
                ...
                write (stdout,'(a,"-",a,"=",f4.2," ")',advance="no") &
                        trim(species_names(na)),trim(species_names(nb)),0.0_dp ! * Fully connected
             end if
  ...
  subroutine rank()
     ...
     ctemp=''
     if(have_spin) ctemp='     S   |S|'
     ctemp2=''
     if(have_dos) ctemp2='     dos'
     
     if(long) then
        
        write (stderr,'(a,a)') 'structure                                   P/GPa     V/A^3        ',&
              'H/eV'//trim(ctemp)//trim(ctemp2)//'  nfu formula      space group    #'
     else
        write (stderr,'(a,a)') 'structure               P/GPa     V/A^3        ',&
              'H/eV'//trim(ctemp)//trim(ctemp2)//'  nfu formula      space group    #'
       ...
       if(structure(sort_index(ns))%num_copies>0) then
          ...
          if(have_dos) then
             if(structure(sort_index(ns))%del.ge.0.0_dp) then
                write(ctemp2,'(f8.3)') structure(sort_index(ns))%del/structure(sort_index(ns))%num_form
             else
                ctemp2='      '
             end if
          end if
          ...
          if(long) then
             fmt='(a40,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a11,i5)'
             label=structure(sort_index(ns))%structure_label
          else
             fmt='(a20,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a11,i5)'
  ...
  subroutine summary()
    ...
    allocate(cvec(num_species,num_compositions),cvec2(num_species,num_compositions),disclap(num_compositions))
    ...
    do nc=1,num_compositions
       ...
       if(have_dos) then
          if(structure(indx)%del.ge.0.0_dp) then
             write(ctemp2,'(f7.3)') structure(indx)%del/structure(indx)%num_form
          else
             ctemp2='      '
          end if
       end if
       ...
       if(cluster) then
          ...
          if(long) then
             fmt='(a40,f12.3,1x,3a,1x,a12,1x,a11,i5,i6)'
             label=structure(indx)%structure_label
          else
             fmt='(a20,f12.3,1x,3a,1x,a12,1x,a11,i5,i6)'
          ...
          if(long) then
             fmt='(a40,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a7,i5,i6)'
             label=structure(indx)%structure_label
          else
             fmt='(a20,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a7,i5,i6)'
             label=structure(indx)%structure_label_short
          end if
    ...
    if(cluster) then
       ...
       ! ** Ignore constant compositions
       
       cvec2=cvec
     
       nn=0
       do n=1,num_species
          if(.not.(all(cvec2(n,:).eq.cvec2(n,1)))) then
             nn=nn+1
             cvec(nn,:)=cvec2(n,:)
          end if
       end do
...
!!$                 end if
  ...
  subroutine compare()
    ...
    call calculate_distances(seed_indx,rmax,zweight=.true.)
    ...
    do nb=1,num_structures
       ...
       call calculate_distances(nb,rmax,zweight=.true.)
       ...
       do n=1,min(structure(nb)%num_dist*structure(seed_indx)%num_form,structure(seed_indx)%num_dist*structure(nb)%num_form)
          ...
          nii=(n-1)/structure(nb)%num_form+1
          nbb=(n-1)/structure(seed_indx)%num_form+1

          if(structure(seed_indx)%distall(nii)<rmax/1.75_dp) then
             
             diff=abs(structure(nb)%distall(nbb)*scale_b-structure(seed_indx)%distall(nii)*scale_a) 
       ...
       if(maxdiff(nb,seed_indx).lt.comp_threshold) then
          ...
          if(have_dos) then
             if(structure(nb)%del.ge.0.0_dp) then
                write(ctemp2,'(f7.3)') structure(nb)%del/structure(nb)%num_form
             else
                ctemp2='      '
             end if
          end if
          ...
          if(long) then
             fmt='(a40,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a11,i5,f10.4)'
             label=structure(nb)%structure_label
          else
             fmt='(a20,f9.2,f10.3,f12.3,2a,i5,1x,a12,1x,a11,i5,f10.4)'
  ...
  subroutine eliminate()
    ...
    do na=1,num_structures
       ...
       if((structure(nas)%num_copies.gt.0).and.(elim_threshold.gt.0.0_dp)) then
          call calculate_distances(nas,rmax,zweight=.true.)           
       ...
       do na=1,num_structures
          ...
          do nb=na+1,num_structures
             ...
             do n=1,min(structure(nas)%num_dist*structure(nbs)%num_form,structure(nbs)%num_dist*structure(nas)%num_form)
                ...
                naa=(n-1)/structure(nbs)%num_form+1
                nbb=(n-1)/structure(nas)%num_form+1
                
                if(structure(nas)%distall(naa)<rmax/1.75_dp) then

                   diff=abs(structure(nas)%distall(naa)*scale_a-structure(nbs)%distall(nbb)*scale_b)
                   
                   if(diff>elim_threshold*(minda*scale_a+mindb*scale_b)/2.0_dp) then
                      similar(nbs)=0         
                      exit
                   end if
  ...
  subroutine maxwell()
     ...
     integer :: i,j,k,l,num_1,num_2,num_3,num_4,ndupe
     
     character(len=40), allocatable, dimension(:,:) :: hull_comp
     
     character(len=40) :: ctemplong
     character(len=20) :: ctempshort
    ...
    ! ** Identify requested convex hulls
    
    num_1=count(len_trim(comp_endmember(:,1)).gt.0)
    num_2=count(len_trim(comp_endmember(:,2)).gt.0)
    num_3=count(len_trim(comp_endmember(:,3)).gt.0)
    num_4=count(len_trim(comp_endmember(:,4)).gt.0)
    
    allocate(hull_comp(4,(num_1+1)*(num_2+1)*(num_3+1)*(num_4+1)))
    
    num_hulls=1
    
    hull_comp=''
        
    if(any(len_trim(comp_endmember).gt.0)) then
                       
       if(num_2.eq.0) stop 'cryan: maxwell, insufficient end members defined'
              
       if(num_3.eq.0) then
          
          num_hulls=0
          do i=1,num_1
             comp_1=comp_endmember(i,1)
             do j=1,num_2
                comp_2=comp_endmember(j,2)
                if(comp_1.eq.comp_2) cycle
                
                ! * Check for duplicates

                dupe=.false.
                do nh=1,num_hulls
                   if(the_same((/comp_1,comp_2/),hull_comp(1:2,nh))) dupe=.true.
                end do                
                ...
                num_hulls=num_hulls+1
                
                hull_comp(1:2,num_hulls)=(/comp_1,comp_2/)
          ...
          if(num_4.eq.0) then
             ...
             do i=1,num_1
                ...
                do j=1,num_2
                   ...
                   if(comp_1.eq.comp_2) cycle 
                   do k=1,num_3
                      comp_3=comp_endmember(k,3)
                      if(comp_1.eq.comp_3) cycle 
                      if(comp_2.eq.comp_3) cycle 
                      
                      ! * Check for duplicates

                      dupe=.false.
                      do nh=1,num_hulls
                         if(the_same((/comp_1,comp_2,comp_3/),hull_comp(1:3,nh))) dupe=.true.
                      end do                
                      ...
                      hull_comp(1:3,num_hulls)=(/comp_1,comp_2,comp_3/)
             ...
             do i=1,num_1
                ...
                do j=1,num_2
                   ...
                   do k=1,num_3
                      ...
                      if(comp_2.eq.comp_3) cycle 
                      do l=1,num_4
                         comp_4=comp_endmember(l,4)
                         if(comp_1.eq.comp_4) cycle 
                         if(comp_2.eq.comp_4) cycle 
                         if(comp_3.eq.comp_4) cycle 
                         
                         ! * Check for duplicates

                         dupe=.false.
                         do nh=1,num_hulls
                            if(the_same((/comp_1,comp_2,comp_3,comp_4/),hull_comp(1:4,nh))) dupe=.true.
                         end do                
                         ...
                         hull_comp(1:4,num_hulls)=(/comp_1,comp_2,comp_3,comp_4/)
    ...
    do nh=1,num_hulls
       ...
       comp_1=hull_comp(1,nh)
       comp_2=hull_comp(2,nh)
       comp_3=hull_comp(3,nh)
       comp_4=hull_comp(4,nh)
                     
       hull_prefix='null'
                     
       if(allocated(cmat)) deallocate(cmat,comp_vec,comp_frac,comp_energy,point_energy,comp_include,&
          comp_factor,comp_onhull,comp_efromhull,vec,hull_index,hull_normal)
       if(allocated(tri_index)) deallocate(tri_index)
    ...
    if((len_trim(comp_1).gt.0).and.(len_trim(comp_2).gt.0)) then
       ...
       if  (len_trim(comp_4).gt.0) then
          ...
          hull_prefix=trim(comp_1)//"-"//trim(comp_2)//"-"//trim(comp_3)//"-"//trim(comp_4)
       ...
       elseif (len_trim(comp_3).gt.0) then
          ...
          hull_prefix=trim(comp_1)//"-"//trim(comp_2)//"-"//trim(comp_3)
          ...
          hull_prefix=trim(comp_1)//"-"//trim(comp_2)
    ...
    if(num_points.lt.2) cycle

    if(long) then
       write(ctemplong,'(a)') trim(hull_prefix)
       write (stderr,'(a40,a,a)') ctemplong,'    P/GPa     V/A^3        ',&
       'H/eV form/eV  off/eV  nfu formula      space group    #'
    else
       write(ctempshort,'(a)') trim(hull_prefix)
       write (stderr,'(a20,a,a)') ctempshort,'    P/GPa     V/A^3        ',&
       'H/eV form/eV  off/eV  nfu formula      space group    #'
    end if

    qconv_in=trim(hull_prefix)//"-qconvex.in"
    qconv_out=trim(hull_prefix)//"-qconvex.out"

    open(unit=unit_qhull,form="formatted",file=qconv_in)
    ...
    do nc=1,num_compositions
       ...
       if(sum(abs(comp_vec(:,nc))).lt.delta) comp_include(nc)=.false.
    ...
    do nc=1,num_compositions
       ...
       if(count(comp_vec(:,nc).gt.1e-4_dp).eq.1) then
    ...
    if(num_points.gt.2) then
       ctemp="qconvex i Qt Fx n < "//trim(qconv_in)//" > "//trim(qconv_out)
    else
       ctemp="qconvex Fx n < "//trim(qconv_in)//" > "//trim(qconv_out)
    end if
    stat=0
    call system(ctemp)
    ctemp='Problem executing external command :: '//trim(ctemp)
    ...
    open(unit=unit_qhull,form="formatted",file=qconv_out)
    if(num_points.gt.2) then
    ...
    do nc=1,num_compositions
       ...
       do ns=1,num_structures
          ...
          do n=2,num_normal
             ...
             if(abs(hull_normal(num_points,n)).gt.1e-4_dp) then
          ...
          if((structure(ns)%e_from_hull).gt.(delta_e+1e-4_dp)) structure(ns)%num_copies=0

          if((structure(ns)%e_from_hull).lt.(1e-4_dp)) comp_onhull(ninc)=.true.
    ...
    do nc=1,num_compositions
       ...
       if(long) then
          fmt='(a40,f9.2,f10.3,f12.3,2f8.3,a2,i3,1x,a12,1x,a11,i5)'
          label=structure(indx)%structure_label
       else
          fmt='(a20,f9.2,f10.3,f12.3,2f8.3,a2,i3,1x,a12,1x,a11,i5)'
          label=structure(indx)%structure_label_short
       end if
    ...
    if(num_points.eq.2) then
       ...
       if(xmgrace) then
          ctemp="xmgrace -geometry 1180x920+0+0 "//trim(hull_prefix)//"-hull.agr &"
          stat=0
          call system(ctemp)
          ctemp='Problem executing external command :: '//trim(ctemp)
          if (stat.ne.0) then
       ...
       if (gracefile) then
          ctemp="gracebat "//trim(hull_prefix)//"-hull.agr -hdevice "//trim(grace_output_type)//&
          " -hardcopy -printfile "//trim(hull_prefix)//"-hull."//trim(grace_output_suffix)
          stat=0
          call system(ctemp)
          ctemp='Problem executing external command :: '//trim(ctemp)
          if (stat.ne.0) then
          ...
          print '(a)', "Saved file "//trim(hull_prefix)//"-hull."//trim(grace_output_suffix)
       endif
       ...
       if(xmgrace) then
          ctemp="xmgrace -geometry 1180x920+0+0 "//trim(hull_prefix)//"-ternary.agr &"
          stat=0
          call system(ctemp)
          ctemp='Problem executing external command :: '//trim(ctemp)
          if (stat.ne.0) then
       ...
       if (gracefile) then
          ctemp="gracebat "//trim(hull_prefix)//"-ternary.agr -hdevice "//trim(grace_output_type)//&
          " -hardcopy -printfile "//trim(hull_prefix)//"-ternary."//trim(grace_output_suffix)
          stat=0
          call system(ctemp)
          ctemp='Problem executing external command :: '//trim(ctemp)
          if (stat.ne.0) then
          ...
          print '(a)', "Saved file "//trim(hull_prefix)//"-ternary."//trim(grace_output_suffix)
       endif
  ...
  subroutine phull()
    ...
    do nc=1,num_compositions
       ...
       if((formula.ne.'').and.(formula.ne.composition(nc))) cycle

       write (stderr,'(2a)') 'Composition: ',composition(nc)

       qconv_in=trim(composition(nc))//"-qconvex.in"
       qconv_out=trim(composition(nc))//"-qconvex.out"
       ...
       ctemp="qconvex Fx n < "//trim(qconv_in)//" > "//trim(qconv_out)
       stat=0
       if(num_struc.gt.1) then
          call system(ctemp,stat)
       else
          stat=999
       end if
       if (stat.ne.0) then
          ctemp='Problem executing external command :: '//trim(ctemp)
          if(stat.ne.999) write (stderr,'(a)') trim(ctemp)
       else
          ...
          do na=1,num_structures
             ...
             do n=2,num_normal
                ...
                if(abs(hull_normal(2,n)).gt.1e-4_dp) then
             ...
             if((structure(na)%e_from_hull).gt.(delta_e+1e-4_dp)) structure(na)%num_copies=0

             if((structure(na)%e_from_hull).lt.(1e-4_dp)) then
          ...
          do na=1,num_structures
             ...
             if(long) then
                fmt='(a40,f9.2,f10.3,f11.3,f8.3,a2,i3,1x,a12,1x,a11,i5)'
                label=structure(na)%structure_label
             else
                fmt='(a20,f9.2,f10.3,f11.3,f8.3,a2,i3,1x,a12,1x,a11,i5)'
  ...
  subroutine dos()
    ...
    do nc=1,num_compositions
       ...
       do ns=1,num_structures
          ...
          n=n+1
          etemp=structure(ns)%enthalpy/structure(ns)%num_form
          call  add_gaussian(etemp,egrid,sdos,dos_smear,real(structure(ns)%num_copies,dp))
       end do

       sdos=sdos/real(n,dp)

       call write_dos(composition(nc),egrid,sdos)
  ...
  subroutine write_enthalpy()
    ...
    if (gracefile) then
       ctemp="gracebat "//trim(composition(nc))//"-enthalpy.agr -hdevice "//trim(grace_output_type)//" -hardcopy -printfile "&
            //trim(composition(nc))//"-enthalpy."//trim(grace_output_suffix)
       stat=0
       call system(ctemp)
       ctemp='Problem executing external command :: '//trim(ctemp)
       if (stat.ne.0) then
       ...
       print '(a)', "Saved file "//trim(composition(nc))//"-enthalpy."//trim(grace_output_suffix)
    endif

  end subroutine write_enthalpy

  subroutine write_dos(comp,energy,sd)

    character(len=40),           intent(in) :: comp
    ...
    open(unit=unit_dos,form='formatted',file=trim(comp)//'-struc-dos.agr')
    ...
    if(xmgrace) then
       ctemp="xmgrace -geometry 1180x920+0+0 "//trim(comp)//"-struc-dos.agr &"
       stat=0
       call system(ctemp)
       ctemp='Problem executing external command :: '//trim(ctemp)
       if (stat.ne.0) then
    ...
    if (gracefile) then
       ctemp="gracebat "//trim(comp)//"-struc-dos.agr -hdevice "//trim(grace_output_type)//&
            " -hardcopy -printfile "//trim(comp)//"-struc-dos."//trim(grace_output_suffix)
       stat=0
       call system(ctemp)
       ctemp='Problem executing external command :: '//trim(ctemp)
       if (stat.ne.0) then
       ...
       print '(a)', "Saved file "//trim(comp)//"-struc-dos."//trim(grace_output_suffix)
    endif
  ...
  subroutine write_hull()
    ...
    open(unit=unit_hull,form='formatted',file=trim(hull_prefix)//'-hull.agr')
    ...
    cforma=trim(adjustl(structure(comp_index_inc(hull_index(num_points+1)))%formula))
    cformb=trim(adjustl(structure(comp_index_inc(hull_index(num_hull)))%formula))
    ...
    do n=num_points+1,num_hull
       ...
       cform=trim(adjustl(structure(comp_index_inc(hull_index(n)))%formula))
       csymm=trim(adjustl(structure(comp_index_inc(hull_index(n)))%symmetry))
       write(unit_hull,'(a,a10,i2,1x,a11,f6.3,a)') '@    string def "',&
            cform,&
  ...
  subroutine write_ternary()
    ...
    open(unit=unit_ternary,form='formatted',file=trim(hull_prefix)//'-ternary.agr')
...
!!$       if(comp_energy(ninc).gt.delta) 
    ...
    open(unit=unit_ternary,form='formatted',file=trim(hull_prefix)//'-ternary.r')
    ...
    write(unit_ternary,*) 'suppressWarnings(ggtern(data = maxwell, aes(x = '//trim(comp_1)//&
         ', y = '//trim(comp_2)//', z = '//trim(comp_3)//')) +'
    !write(unit_ternary,*) 'geom_density2d(aes(weight=Energy,color=abs(..level..))) +'
    do n=1,num_tri
       if(any(tri_index(:,n).lt.3)) cycle
       write(unit_ternary,*) 'geom_segment(aes(x=',comp_frac(1,tri_index(1,n)-2),&
            ',y=',comp_frac(2,tri_index(1,n)-2),&
            ',z=',comp_frac(3,tri_index(1,n)-2),&
            ',xend=',comp_frac(1,tri_index(2,n)-2),&
            ',yend=',comp_frac(2,tri_index(2,n)-2),&
            ',zend=',comp_frac(3,tri_index(2,n)-2),&
            '),size=0.2,linetype=1,colour="darkgreen",alpha=0.33) +'
       write(unit_ternary,*) 'geom_segment(aes(x=',comp_frac(1,tri_index(2,n)-2),&
            ',y=',comp_frac(2,tri_index(2,n)-2),&
            ',z=',comp_frac(3,tri_index(2,n)-2),&
            ',xend=',comp_frac(1,tri_index(3,n)-2),&
            ',yend=',comp_frac(2,tri_index(3,n)-2),&
            ',zend=',comp_frac(3,tri_index(3,n)-2),&
            '),size=0.2,linetype=1,colour="darkgreen",alpha=0.33) +'
       write(unit_ternary,*) 'geom_segment(aes(x=',&
            comp_frac(1,tri_index(3,n)-2),&
            ',y=',comp_frac(2,tri_index(3,n)-2),&
            ',z=',comp_frac(3,tri_index(3,n)-2),&
            ',xend=',comp_frac(1,tri_index(1,n)-2),&
            ',yend=',comp_frac(2,tri_index(1,n)-2),&
  ...
  subroutine write_quaternary()
    ...
    open(unit=unit_quaternary,form='formatted',file=trim(hull_prefix)//'-quaternary.plot')
...
!!$    character(len=10) :: cform
    ...
    do na=1,num_structures
       ...
       csymm=trim(adjustl(structure(na)%symmetry))
       write(ctext(n),'(f7.3,i3,1x,a11,a)') &
            structure(na)%enthalpy/structure(na)%num_form-emin,&
            structure(na)%num_form,&
            csymm,&
            '"'
    ...
    do i=2,n
       ...
       if(abs(pest(i)).lt.huge(1.0_dp)) then

          if(abs(pest(i)).gt.0.0_dp) then
          ...
          if(abs(pest(i)).lt.0.5_dp) then
             write(unit_phull,'(a,f7.1,a)') '@    string def "',pest(i)*1000.0_dp,' kPa "'
          else if(abs(pest(i)).lt.500.0_dp) then
             write(unit_phull,'(a,f7.1,a)') '@    string def "',pest(i),' GPa "'
          else
             write(unit_phull,'(a,f7.1,a)') '@    string def "',pest(i)/1000.0_dp,' TPa "'
          end if
    ...
    if (gracefile) then
       ctemp="gracebat "//trim(composition(nc))//"-phull.agr -hdevice "//trim(grace_output_type)//&
            " -hardcopy -printfile "//trim(composition(nc))//"-phull."//trim(grace_output_suffix)
       stat=0
       call system(ctemp)
       ctemp='Problem executing external command :: '//trim(ctemp)
       if (stat.ne.0) then
       ...
       print '(a)', "Saved file "//trim(composition(nc))//"-phull."//trim(grace_output_suffix)
    endif
  ...
  subroutine output_vector()
    ...
    character(len=40) :: tsvFormat = '(*(G0.6,:,"'//achar(9)//'"))'

    integer, parameter :: nbins=1001

    real(kind=dp) :: vecbin(nbins),dr
    
    integer :: nc,nd,nb

    ! ***************************************************
    ! ** Output a descriptor vector for the structures **
    ! ***************************************************
    ...
    do ns=1,num_structures
       ...
       if(structure(ns)%num_copies>0) then
          ...
          call calculate_distances(ns,rmax,zweight=.true.)
          
!!$          dr=rmax/real(nbins-1,dp)
!!$          vecbin=0.0_dp
!!$          do nd=1,structure(ns)%num_dist
!!$             nb=int(structure(ns)%distall(nd)/dr)+1
!!$             vecbin(nb:)=vecbin(nb:)+1.0_dp
!!$          end do
!!$          vecbin=vecbin/real(structure(ns)%num_ions,dp)/real(nbins,dp)
!!$          write (stdout,tsvFormat) vecbin

          write (stdout,tsvFormat) structure(ns)%num_dist,structure(ns)%distall(1:structure(ns)%num_dist)

          write (stdout,tsvFormat) trim(adjustl(structure(ns)%structure_label)),structure(ns)%num_ions,&
               trim(adjustl(structure(ns)%formula)),'"'//trim(adjustl(structure(ns)%symmetry))//'"',&
               structure(ns)%volume,structure(ns)%enthalpy,structure(ns)%num_copies
          
          call deallocate_distances(ns)
  ...
  subroutine get_arguments()
    ...
    integer :: iargc,num_args,na,nele

    character(len=240), allocatable, dimension(:) :: argbuff
...
117       lscale=1.0_dp
       case ('-f','--formula')
          ...
          na=na+1
          formula=argbuff(na)
       case ('-fu','--formula_unit')
          num_tasks=num_tasks+1
          task(num_tasks)='formula_unit'
          read(argbuff(na+1),*,err=122,end=122) formula_unit
          na=na+1
          cycle
122       formula_unit=0
...
114       usage=.true.
       ...
       case('-xg','--xmgrace')
          xmgrace=.true.
       ! read in output file type
       case('-xgf','--xg-fileout')
          gracefile=.true.
          na=na+1
          read(argbuff(na), *, err=132) grace_output_type
          ! Choose correct strings for '-hdevice' and file suffix
          select case (grace_output_type)
             case("ps")
                grace_output_type = "PostScript"
                grace_output_suffix = "ps"
             case("png", "eps", "jpeg", "svg", "pdf")
                grace_output_suffix = grace_output_type
                grace_output_type = trim(up(grace_output_type))
             case DEFAULT
                print '(A,A)', "ERROR Unknown format for grace output: ", grace_output_type
                goto 132
          end select
          cycle
132       usage=.true.
       case('-1')
          na=na+1
          n=1
          do i=1,len(argbuff(na))
             if(argbuff(na)(i:i)==',') n=n+1
          end do
          read(argbuff(na),*,err=106) comp_endmember(1:n,1)
          cycle
106       usage=.true.
       case('-2')
          na=na+1
          ...
          read(argbuff(na),*,err=107) comp_endmember(1:n,2)
          cycle
107       usage=.true.
       case('-3')
          na=na+1
          ...
          read(argbuff(na),*,err=113) comp_endmember(1:n,3)
          cycle
113       usage=.true.
       case('-4')
          na=na+1
          ...
          read(argbuff(na),*,err=115) comp_endmember(1:n,4)
          cycle
115       usage=.true.
       case ('-v','--vector')
          num_tasks=num_tasks+1
          task(num_tasks)='vector'
          vector=.true.
...
121       usage=.true.
       case ('-el','--elementlist')
          na=na+1
          nele=count(transfer(argbuff(na), 'a', len(argbuff(na))) == ",")+1
          read(argbuff(na),*,end=123,err=123) element_list(1:nele)
          cycle
123       usage=.true.
       case ('-sn','--speciesnumber')
          na=na+1
          read(argbuff(na),*,end=124,err=124) specnum
          cycle
124       usage=.true. 
    ...
    if(usage) then
       ...
       print '(a)', ' -nr, --not_relative                  Absolute enthalpy for ranking' 
       ...
       print '(a)', ' -fu, --formula_unt <nfu>             Select structures of a given number of formula units' 
       print '(a)', ' -fc, --formula_convert <formula>     Attempt to convert structures to this composition' 
       print '(a)', ' -fr, --formula_reference <formula>   Structure name for enthalpy reference' 
       print '(a)', ' -sn, --speciesnumber [num            Number of species, e.g 2 for binary' 
       ...
       print '(a)', ' -<n>                                 Component <n> for Maxwell construction' 
       print '(a)', ' -el, --elementlist                   Comma separated list of elements' 
       print '(a)', ' -xg, --xmgrace                       Plot output with xmgrace' 
       print '(a)', ' -xgf, --xg-fileout <format>          Save output plot in one of the formats: png, jpeg, svg, eps, ps, pdf' 
  ...
  subroutine calculate_distances(num,rmax,nosort,zweight,laplacian,superlap)

    integer,                                 intent(in)  :: num
    real(kind=dp),                           intent(in)  :: rmax
    logical,                       optional, intent(in)  :: nosort
    logical,                       optional, intent(in)  :: zweight
    ...
    real(kind=dp) :: veca(3),vecb(3),dist2,thresh2,lap2,z,zmax

    logical :: sort,lap,zwgt
    ...
    do na=1,structure(num)%num_ions
       z=get_z(structure(num)%ion_names(na))
       if(z.gt.zmax) zmax=z
    end do
...
102 n1max=0 ; n2max=0 ; n3max=0 ; n1min=0 ; n2min=0 ; n3min=0
    ...
    if(.not.allocated(structure(num)%distall)) then
       ...
       allocate(structure(num)%dist_Z(2,structure(num)%num_dist*structure(num)%num_ions))
             ...
             do n2=n2min,n2max
                do n3=n3min,n3max
                   ...
                   if(dist2<thresh2) then
                      ...
                      structure(num)%dist_Z(1,nn) = get_z(structure(num)%ion_names(na))                     
                      structure(num)%dist_Z(2,nn) = get_z(structure(num)%ion_names(nb))
                      if(zwgt) structure(num)%distall(nn) = structure(num)%distall(nn)*zmax**2/product(structure(num)%dist_Z(:,nn))
...
103    n1max=0 ; n2max=0 ; n3max=0 ; n1min=0 ; n2min=0 ; n3min=0
  ...
  subroutine deallocate_distances(num)

    integer,  intent(in)  :: num

    if(allocated(structure(num)%distall)) then
       deallocate(structure(num)%distall)
       deallocate(structure(num)%dist_names)
       deallocate(structure(num)%dist_Z)
       deallocate(structure(num)%dist_ions)
       deallocate(structure(num)%dist_incell)
       deallocate(structure(num)%dist_indx)
    end if

    if(allocated(structure(num)%laplacian)) then
       deallocate(structure(num)%laplacian)
       deallocate(structure(num)%adjacancy)
    end if

  end subroutine deallocate_distances
  ...
  function vec2decomp(M,V,factor)
    ...
    real(kind=dp), dimension(size(M(1,:))) :: vec2decomp

    logical :: success

    ! --

    call linear_solve(M,V,vec2decomp,success)
...
20     if(j.le.ir) then
  ...
  subroutine linear_solve(Aint,Bint,D,success)
    ...
    ! ** Check we got a good solution

    if((norm2(matmul(Aint,B(1:n,1))-Bint).lt.delta).and.(all(B(1:n,1).gt.-delta))) then
       D=B(1:n,1)
       success=.true.
    else
       D=0.0_dp
  ...
  function get_z(element)
    ...
    if(index(element,"'").gt.0) then
       ele=element(:index(element,"'")-1)
    else
       ele=element
    end if
    
    if(.not.any(elements_alpha.eq.ele)) stop 'cryan: get_z: element not recognised'
    
    z=0
    do
       z=z+1
       if(elements_alpha(z).eq.ele) exit
    end do
  ...
  ! Function to make characters upper case
  function up(string)
  ...
  function the_same(a,b)
     ...
     ! * Assume duplicates in single string already removed
