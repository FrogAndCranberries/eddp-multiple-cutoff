...
!!$  end interface
  ...
  real(kind=dp), public, dimension(:),     allocatable :: ion_vol,ion_vol_temp
  ...
  real(kind=dp), public, dimension(:,:),   allocatable :: ion_set_vol
  ...
  subroutine by_species_cell()
    ...
    if(nspec.gt.0) then
       ...
       if(natom.gt.0) then
          ...
          do na=1,natom
             write(ctemp2,*) na
             cellbuff(n+1+na)=trim(cspec(atom2spec(na)))//' 0.0 0.0 0.0 # '//trim(adjustl(ctemp2))//' % '//trim(cmod(atom2spec(na)))
          end do
...
!!$    write (stderr,*) '---'
  ...
  subroutine choose_range()

    character(len=100) :: cwork,ctemp

    real(kind=dp) :: mn,mx,rn

    integer :: indx

    do n=1,nrecords

       if(index(up(cellbuff(n)),'VOL=').gt.0) then

          ! ** This will pick up VOL, TARGVOL, VARVOL
          
          if(index(cellbuff(n)(index(cellbuff(n),'VOL=')+4:),'-').gt.0) then
             cwork=cellbuff(n)(index(cellbuff(n),'VOL=')+4:)
             
             read (cwork(:index(cwork,'-')-1),*) mn
             read (cwork(index(cwork,'-')+1:),*) mx

             cwork=trim(cwork(index(cwork,' '):))
             
             rn=mn+random_single()*(mx-mn)

             write (ctemp,*) rn

             cellbuff(n)=cellbuff(n)(:index(cellbuff(n),'VOL=')+3)//trim(adjustl(ctemp))//trim(cwork)
             
          end if

       end if
       
       if(index(up(cellbuff(n)),'AMP=').gt.0) then

          ! ** This will pick up POSAMP, MINAMP, CELLAMP etc
          
          if(index(cellbuff(n)(index(cellbuff(n),'AMP=')+4:),'-').gt.0) then
             cwork=cellbuff(n)(index(cellbuff(n),'AMP=')+4:)
             
             read (cwork(:index(cwork,'-')-1),*,err=99,end=99) mn
             read (cwork(index(cwork,'-')+1:),*) mx
             ...
             cellbuff(n)=cellbuff(n)(:index(cellbuff(n),'AMP=')+3)//trim(adjustl(ctemp))//trim(cwork)
...
99           continue   
  ...
  subroutine inject_smiles()
    ...
    character(len=100) :: smilabel,smimore,nbscale

    real(kind=dp) :: position(3),shift(3)
    
    logical :: conformer,isomer
...
654          read(cellbuff(m),*) ctemp
             ...
             if(index(ctemp,'SMI:').gt.0) then
                ...
                isomer=index(up(smimore),'ISOMER').gt.0
                nbscale="0.75"
                if(index(up(smimore),'NONBOND')>0) read(smimore(index(up(smimore),'NONBOND')+8:),*) nbscale
...
!!$                        '" -oxyz | obabel -ixyz --conformer --random --nconf 1 -oxyz -O '//trim(ctemp2)//' 2>/dev/null'
                   ...
                   ctemp6='obabel --ff uff -c --gen3d -:"'//trim(ctemp4)//&
                        '" -oxyz | muddle -nb '//trim(nbscale)//' > '//trim(ctemp2)//' 2>/dev/null'

                elseif(isomer) then

                    ctemp6='obabel --ff uff -c --gen3d -:"'//trim(ctemp4)//&
                        '" -oxyz | muddle -s -nb '//trim(nbscale)//' > '//trim(ctemp2)//' 2>/dev/null'
                ...
                do i=0,nn-1
                   ...
                   if(i.eq.0) then
                      ...
                      cellbuff(m+i)=trim(ctemp4)//' '//trim(ctemp6)//' '//trim(smilabel)//trim(smimore(:index(smimore,"%")))
                   end if
                end do
  ...
  subroutine init_cell(stat)
    ...
    if(allocated(ion_new_positions))  deallocate(ion_new_positions,ion_temp_positions,ion_hold_positions,ion_nn,ion_fix,ion_perm,&
         ...
         xamp_temp,yamp_temp,zamp_temp,ion_athole,ion_athole_temp,ion_vol,ion_vol_temp)
          ...
          do i=1,num_ions
             ...
             if(index(ctemp,'#')>0) then
                ...
                if(index(ctemp,'%')>0) then
                   ...
                   if(index(up(ctemp),'VOL')>0)    read(ctemp(index(up(ctemp),'VOL')+4:),*) ion_vol(i)
    ...
    ! ** Set ADATOM for whole set

    if(any(ion_adatom)) then

       do n=1,num_ions

          do m=1,num_ions

             if(ion_set(n)==ion_set(m)) then
                ion_adatom(n)=ion_adatom(m)
                exit
             end if
...
456          close(unit=unit_minsep)
          ...
          ctemp4=adjustl(trim(ctemp(index(up(ctemp),'#MINSEP=')+8:)))
          ctemp5=adjustl(trim(ctemp4(:index(up(ctemp4),' '))))
          ctemp4=adjustl(ctemp4(index(ctemp4,ctemp5)+len_trim(ctemp5)+1:))

          ! First try to read simple range

          if (index(ctemp5,"-").gt.0) then
             read(ctemp5(:index(ctemp5,"-")-1),*,err=521,end=521) minmin
             read(ctemp5(index(ctemp5,"-")+1:),*,err=521,end=521) minmax
             minsep=minmin
          else
             read(ctemp5,*,err=521,end=521) minmin
             minmax=minmin
             minsep=minmin
          end if

521       ctemp4=' '//ctemp4 !! Add in leading space
...
88        read(ctemp2,*) spin,spinmod
    ...
    ! ** If all ion volumes are set, use them to set the volume

    if(all(ion_vol(1:num_ions).gt.0.0_dp)) then
       targvol=sum(ion_vol(1:num_ions))
       fix_vol=.true.
    end if
...
999 write (stderr,*) 'There is a problem reading the cell information. Stopping.'
    ...
    subroutine gen_sets()
      ...
      if(allocated(set_label)) then
         ...
         deallocate(ninset,ion_set_min,posamp_set,minamp_set,angamp_set,ion_set_vol)
      ...
      allocate(ion_set_vol(num_ions,num_sets))
      ...
      do n=1,num_ions
         ...
         do m=1,num_sets
            ...
            if(ion_set(n)==set_label(m)) then
               ...
               ion_set_vol(ninset(m),m) = ion_vol(n)
