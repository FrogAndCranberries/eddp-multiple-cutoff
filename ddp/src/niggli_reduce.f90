...
subroutine niggli_reduce(Lm,Cm)
  ...
  real(kind=gp),parameter :: eps=1e-10_gp

  integer       :: Cc(3,3)

  real(kind=gp) :: A,B,C,xi,eta,zeta
...
contains
  ...
  function step0()
    ...
    xi=  dot_product(Lm(:,2),Lm(:,3))*2.0_gp
    eta= dot_product(Lm(:,3),Lm(:,1))*2.0_gp
    zeta=dot_product(Lm(:,1),Lm(:,2))*2.0_gp
  ...
  function step5()
    ...
    if ((abs(xi).gt.B+eps).or.(.not.(abs(B-xi).gt.eps).and.(2.0_gp*eta.lt.zeta-eps))&
       ...
       Cc(2,2)=1
       Cc(3,3)=1
       if(xi.gt.0.0_gp) Cc(3,2)=-1  
       if(xi.lt.0.0_gp) Cc(3,2)=1  
  ...
  function step6()
    ...
    if ((abs(eta).gt.A+eps).or.(.not.(abs(A-eta).gt.eps).and.(2.0_gp*xi.lt.zeta-eps))&
       ...
       Cc(2,2)=1
       Cc(3,3)=1
       if(eta.gt.0.0_gp) Cc(3,1)=-1  
       if(eta.lt.0.0_gp) Cc(3,1)=1
  ...
  function step7()
    ...
    if ((abs(zeta).gt.A+eps).or.(.not.(abs(A-zeta).gt.eps).and.(2.0_gp*xi.lt.eta-eps))&
       ...
       Cc(2,2)=1
       Cc(3,3)=1
       if(zeta.gt.0.0_gp) Cc(2,1)=-1  
       if(zeta.lt.0.0_gp) Cc(2,1)=1
  ...
  function step8()
    ...
    if((xi+eta+zeta+A+B.lt.-eps).or.(.not.(abs(xi+eta+zeta+A+B).gt.eps).and.(2.0_gp*(A+eta)+zeta.gt.eps))) then
