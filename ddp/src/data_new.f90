...
module data_new
  ...
  type, public :: metadata
     ...
     real(kind=gp),       public, allocatable, dimension(:)    :: enthalpy,pressure,volume
     real(kind=gp),       public, allocatable, dimension(:,:)  :: rmax
...
contains
  ...
  subroutine data_read(datafile,data,meta)
    ...
    character(len=2000) :: ctemp,cvec(23)
    ...
    do
       ...
       if(cvec(1).eq.'structure:')  then
          ...
          read(cvec(20),*) lcheck 
          read(cvec(21),*) tlcheck
          read(cvec(22),*) flcheck
          read(cvec(23),*) fvlcheck
...
99  rewind(unit_data)
    ...
    allocate(meta%struct(data%num),meta%ncentres(meta%nstruct),meta%label(meta%nstruct),meta%pressure(meta%nstruct),&
         meta%volume(meta%nstruct),meta%enthalpy(meta%nstruct),meta%rmax(meta%nstruct, 5),meta%natoms(meta%nstruct))
    ...
    do ns=1,meta%nstruct
       ...
       read(cvec(12),*) meta%rmax(ns,1)
       read(cvec(13),*) meta%rmax(ns,2)
       read(cvec(14),*) meta%rmax(ns,3)
       read(cvec(15),*) meta%rmax(ns,4)
       read(cvec(16),*) meta%rmax(ns,5)
...
101 stop 'data_read : file not found'
  ...
  function data_compatible(data1,meta1,data2,meta2) result(compat)
    ...
    if(meta1%rmax(1,1).ne.meta2%rmax(1,1))     compat=.false.
    if(meta1%rmax(1,2).ne.meta2%rmax(1,2))     compat=.false.
    if(meta1%rmax(1,3).ne.meta2%rmax(1,3))     compat=.false.
    if(meta1%rmax(1,4).ne.meta2%rmax(1,4))     compat=.false.
    if(meta1%rmax(1,5).ne.meta2%rmax(1,5))     compat=.false.
