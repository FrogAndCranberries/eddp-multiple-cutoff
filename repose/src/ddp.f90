...
module ddp
   ...
   real(kind=gp) :: rcut,rcut_sq,rcore,acore,rmin
...
999   stop 'read_ddp: problem opening/reading eddp file'
   ...
   subroutine ball_ddp(str)
      ...
      nna=int(2*rcut/h(1))+1
      nnb=int(2*rcut/h(2))+1
      nnc=int(2*rcut/h(3))+1
      ...
      do ion_i=1,str%num_ions
         ...
         if(twobody) then
            ...
            do ion_j=1,nball(ion_i)
               ...
               fc12p(:)=fcpow(mod_r12,rcut,pow)

               f12(:)=fcpfx(mod_r12,rcut)*pow(:)
               ...
               if(threebody) then
                  ...
                  do ion_k=ion_j+1,nball(ion_i)
                     ...
                     if(mod_r23_sq.gt.rcut_sq) cycle
                     ...
                     f13(:)=fcpfx(mod_r13,rcut)*pow(:)
                     f23(:)=fcpfx(mod_r23,rcut)*pow(:)

                     fc13p(:)=fcpow(mod_r13,rcut,pow)
                     fc23p(:)=fcpow(mod_r23,rcut,pow)
